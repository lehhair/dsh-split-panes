	...
	factory: (require) => {
		...
		*/
		function absentHook() {
			...
			return ((_selector) => useAbsent(() => void 0));
		}
		/**
		...
		*/
		function buildPaneKit(ctx, sessionId) {
			...
			if (binding !== void 0) {
				...
				for (const [name, source] of Object.entries(binding.hooks)) hooks[name] = source === void 0 ? absentHook() : bindSelector(source);
			...
			for (const name of ["conversation", "input"]) if (hooks[name] === void 0) hooks[name] = absentHook();
			return {
				sessionId,
		...
		*/
		function resolveOccupant(raw) {
			...
			const render = (ctx, renderChild, owner, slotInject, hookContext) => {
				...
				let face = {};
				if (record.inject !== void 0) face = cachedInject(record, sessionId, instance?.actions);
		...
		function storeOf(handle, sessionId) {
		...
		const injectCache = /* @__PURE__ */ new WeakMap();
		/** Resolve (and lazily cache) the entry's injected face for one pane session. */
		function cachedInject(record, sessionId, actions) {
			...
			if (face === void 0) {
				face = bindInjectSources(record.inject(...actions !== void 0 ? [sessionId, actions] : [sessionId]));
		...
		*/
		function createPaneRenderHost(slots, kit, locale) {
			...
			*/
			const renderConversationRoot = () => {
				const rootEntry = slots.entries("conversation").find((raw) => (viewOf(raw).options?.priority ?? 0) > TAKEOVER_PRIORITY$1);
				if (rootEntry === void 0) return null;
				const occupant = resolveOccupant(rootEntry);
				if (occupant === void 0) return null;
				return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PaneBoundary, {
					fallback: null,
					children: occupant.render(mount, renderChild, {}, EMPTY_SLOT_INJECT, void 0)
				});
			};
			return {
		...
		*/
		...
		const PaneConversation = (0, react.memo)(function PaneConversation(props) {
			const { ctx, sessionId } = props;
			const kit = (0, react.useMemo)(() => buildPaneKit(ctx, sessionId), [ctx, sessionId]);
			return (0, react.useMemo)(() => createPaneRenderHost(ctx.slots, kit, ctx.locale), [
				ctx.slots,
				kit,
				ctx.locale
			]).renderConversationRoot();
		...
		*/
		function apply(ctx) {
			...
			*/
			...
			const renderPane = (sessionId, key) => (0, react.createElement)(PaneConversation, {
				key,
				ctx,
				sessionId
			});
