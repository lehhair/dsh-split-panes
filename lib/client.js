window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function buildPaneKit(ctx, sessionId) {
			...
			if (binding !== void 0) {
				...
				for (const [name, source] of Object.entries(binding.keyedHooks)) keyedHooks[name] = source === void 0 ? void 0 : ((key, selector, equal) => {
					return bindSelector(source(key) ?? ABSENT_SOURCE)((value) => selector(value), equal);
				});
			}
			return {
				sessionId,
				...
				useProjection: keyedHooks["projection"] ?? ((_key, selector) => selector(void 0)),
				hooks,
				keyedHooks,
				props
			};
		...
		* appear as they land.
		*/
		function createPaneRenderHost(slots, kit) {
