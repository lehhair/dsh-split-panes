window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const storeInstances = /* @__PURE__ */ new WeakMap();
		...
		function storeOf(entry, binding) {
			...
			let perHandle = storeInstances.get(handle);
			if (perHandle === void 0) {
				perHandle = /* @__PURE__ */ new Map();
				storeInstances.set(handle, perHandle);
			}
			let instance = perHandle.get(key);
			if (instance === void 0) {
				instance = binding === void 0 ? handle.create() : handle.create(key);
		...
		*/
		function ensureSessionOpen(ctx, sessionId) {
			...
			const face = ctx.get("sessions")?.binding(sessionId)?.session;
		...
		*/
		...
		function PaneFrame(props) {
			const { leaf, focused, current, blankIds, renderPane, actions, openSession, splitWithNew, t } = props;
