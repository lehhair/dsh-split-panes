window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function SplitPaneButton({ useStore, useSessions, useWorkspaces, splitWithNew, t }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
				...
				title: t("pane.split"),
				onClick: () => {
					if (focusedPaneId !== null) splitWithNew(focusedPaneId, "horizontal", current ?? null, workspaceId);
				},
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconSplitHorizontal16, {})
			});
		}
		...
		*/
		function SplitVerticalButton({ useStore, useSessions, useWorkspaces, splitWithNew, t }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
				...
				title: t("pane.split.vertical"),
				onClick: () => {
					if (focusedPaneId !== null) splitWithNew(focusedPaneId, "vertical", current ?? null, workspaceId);
				},
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(IconSplitVertical16, {})
			});
		}
		...
		*/
		...
		const css = "[data-phase]>header:not([aria-hidden=true]){padding:var(--pane-pad-top,12px) 160px 0 20px;z-index:5;border-bottom:none;grid-template-columns:1fr auto;align-items:center;gap:16px;display:grid;position:relative}[data-phase]>header:not([aria-hidden=true])>div:first-child{min-width:0}[data-phase]>header:not([aria-hidden=true])>div:nth-child(2){justify-content:flex-end;margin:0;padding:0}[data-phase]>header:not([aria-hidden=true])>div:nth-child(2) button{align-items:center;padding:0 0 4px;display:flex}[data-phase]>header:not([aria-hidden=true])>div:first-child>div:last-child{right:28px;top:calc(50% + calc(var(--pane-pad-top,12px) / 2));position:absolute;transform:translateY(-50%)}[data-phase]>header:not([aria-hidden=true]):after{content:\"\";pointer-events:none;background:linear-gradient(to bottom, var(--dsw-alias-bg-base), transparent);height:32px;position:absolute;top:100%;left:0;right:0}[data-phase]>[data-conversation-scroll]{padding-top:12px}:root{--dsw-specific-sidebar-fill:var(--dsw-static-neutral-bluish-00)}body[data-ds-dark-theme]{--dsw-specific-sidebar-fill:var(--dsw-static-neutral-bluish-950)}";
		...
		*/
		function apply(ctx) {
			...
			*/
			const splitWithNew = (paneId, direction, anchor, workspaceId) => {
				paneStore.actions.splitPane(paneId, direction, anchor);
				const newPaneId = paneStore.getSnapshot().focusedPaneId;
				if (workspaceId !== void 0 && newPaneId !== null) ctx.sessions.create({ workspaceId }).then((sessionId) => {
					paneStore.actions.setPaneSession(newPaneId, sessionId);
				}, () => {});
			};
			ctx.effect(() => {
