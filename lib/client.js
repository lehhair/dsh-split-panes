window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		function bindSelector(source) {
			const subscribe = (fn) => source.subscribe(fn);
			const getSnapshot = () => source.getSnapshot();
			return function useSelect(selector, equal) {
				return selector((0, react.useSyncExternalStore)(subscribe, getSnapshot, getSnapshot));
			};
		}
		...
		function absentHook() {
			...
			return (selector) => useAbsent((_value) => selector(void 0));
		}
		/**
		...
		*/
		function ensurePaneSessionOpen(sessions, binding) {
			if (sessions === void 0 || binding === void 0) return;
			const session = sessions.sessionOf(binding.ctx);
			if (session?.open !== void 0) session.open();
		}
		/**
		...
		*/
		function buildPaneKit(ctx, sessionId) {
			...
			if (workspaces?.list !== void 0) hooks["workspaces"] = bindSelector(workspaces.list);
			if (binding !== void 0) {
				for (const [name, value] of Object.entries(binding.props)) props[name] = value;
				for (const [name, source] of Object.entries(binding.hooks)) hooks[name] = source === void 0 ? void 0 : bindSelector(source);
				for (const [name, source] of Object.entries(binding.keyedHooks)) keyedHooks[name] = source === void 0 ? void 0 : ((key, selector, equal) => {
					return bindSelector(source(key) ?? ABSENT_SOURCE)(((value) => (selector ?? identity)(value)), equal);
				});
			}
			return {
				sessionId,
				...
				useProjection: keyedHooks["projection"] ?? ((key, selector) => (selector ?? identity)(void 0)),
				hooks,
				keyedHooks,
				props
			};
		...
		*/
		...
		const css$4 = ".ytwmKG_host{flex:1;min-width:0;min-height:0;display:flex}.ytwmKG_split{flex:1;min-width:0;min-height:0;display:grid}.ytwmKG_paneSide{min-width:0;min-height:0;display:flex}.ytwmKG_paneSide>*{flex:1;min-width:0;min-height:0}.ytwmKG_divider{z-index:1;touch-action:none;cursor:col-resize;background:0 0;position:relative}.ytwmKG_split[data-direction=vertical] .ytwmKG_divider{cursor:row-resize}.ytwmKG_divider:hover,.ytwmKG_divider:focus-visible{background:var(--dsw-alias-interactive-bg-hover);outline:none}";
		const tagId$4 = "@dsh-external/dsh-split-panes/SplitContainer.module.css";
		...
		*/
		...
		function PaneFrame(props) {
			const { leaf, focused, current, isNewConversation, renderPane, actions, openSession, splitWithNew, t } = props;
		...
		*/
		function PaneWorkspace({ usePaneStore, paneActions, useSessions, openSession, splitWithNew, renderPane, t }) {
			...
			(0, react.useEffect)(() => {
				...
				if (paneId !== void 0) paneActions.setPaneSession(paneId, current);
			}, [current, paneActions]);
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(SplitContainer, {
					...
					renderLeaf: (leaf) => {
						...
						return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PaneFrame, {
							leaf,
		...
		//#endregion
		//#region src/client/render-host.tsx
		/**
		...
		*/
		...
		function bindInjectSources(face) {
			...
			if (sources !== null && typeof sources === "object") {
				for (const [name, source] of Object.entries(sources)) if (source !== void 0) out[(0, _deepseek_ai_dsh_client_ui_slots.standardHookPropName)(name)] = bindSelector(source);
			}
			if (keyed !== null && typeof keyed === "object") for (const [name, source] of Object.entries(keyed)) {
				if (source === void 0) continue;
				const hook = ((key, selector, equal) => {
					return bindSelector(source(key) ?? {
						getSnapshot: () => void 0,
						subscribe: () => () => {}
					})(((value) => (selector ?? identityOf)(value)), equal);
				});
				out[(0, _deepseek_ai_dsh_client_ui_slots.standardHookPropName)(name)] = hook;
			}
			return out;
		}
		/** The identity selector (the keyed hook's no-selector default). */
		const identityOf = (value) => value;
		/** Error boundary so one pane's native occupant cannot take down the workspace. */
		var PaneBoundary = class extends react.Component {
			state = { failed: false };
		...
		*/
		function cachedSlotInject(face) {
			...
			if (definitions === void 0 || typeof definitions !== "object") {
				bound = {
					props: face,
					factories: void 0
				};
			...
			for (const [name, definition] of Object.entries(definitions)) {
				...
				} else props[hookName] = definition;
			}
			bound = {
				props,
				factories
		...
		*/
		function resolveOccupant(raw) {
			const record = viewOf(raw);
			const component = record.component;
			if (typeof component !== "function") return void 0;
			const store = record.store;
			const render = (ctx, renderChild, owner, slotInject, hookContext) => {
				const sessionId = ctx.kit.sessionId;
				const props = { ...ctx.kit.props };
				let face = {};
				if (record.inject !== void 0) {
					const actions = store?.create(sessionId).actions;
					face = bindInjectSources(record.inject(...store !== void 0 ? [sessionId, actions] : [sessionId]));
				}
				...
				for (const [name, hook] of Object.entries(ctx.kit.hooks)) {
					...
					props[name.startsWith("use") ? name : (0, _deepseek_ai_dsh_client_ui_slots.standardHookPropName)(name)] = hook;
				}
				if (record.locale !== void 0 && ctx.locale !== void 0) props["t"] = ctx.locale.bind(record.locale);
				Object.assign(props, slotInject.props);
				if (slotInject.factories !== void 0) for (const [name, factory] of Object.entries(slotInject.factories)) {
					const hook = factory(props, hookContext);
					props[(0, _deepseek_ai_dsh_client_ui_slots.standardHookPropName)(name)] = hook;
				}
				Object.assign(props, owner);
				props["renderSlot"] = renderChild.renderSlot;
				props["renderSlotChain"] = renderChild.renderSlotChain;
				return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PaneBoundary, {
					fallback: null,
					children: (0, react.createElement)(component, props)
				});
			};
			...
			return {
				...
				...record.select !== void 0 ? { select: record.select } : {},
				...record.locale !== void 0 ? { locale: record.locale } : {},
				render
			};
		}
		/**
		...
		* appear as they land.
		*/
		function createPaneRenderHost(slots, kit, locale) {
			...
			const occupants = (key) => {
				...
				if (records.length === 0) resolved = NO_RECORDS;
				else resolved = records.map(resolveOccupant).filter((occupant) => occupant !== void 0);
			...
			let renderChild;
			const dispatch = (key, occupant, owner, opts) => {
				const slotInject = cachedSlotInject((slots.specOf?.(key) ?? slots.spec?.(key))?.inject);
				return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PaneBoundary, {
					fallback: opts?.fallback,
					children: occupant.render(mount, renderChild, owner, slotInject, opts?.hookContext)
				});
			};
			renderChild = {
				renderSlot: (key, owner, opts) => {
					const list = occupants(key);
					if (opts?.entryKey !== void 0) {
						const found = list.find((o) => o.key === opts.entryKey);
						return found === void 0 ? opts.fallback ?? null : dispatch(key, found, owner, opts);
					}
					if (opts?.only !== void 0) {
					...
					const first = list[0];
					return first === void 0 ? opts?.fallback ?? null : dispatch(key, first, owner, opts);
				},
				renderSlotChain: (key, owner, opts) => {
					...
					for (const occupant of occupants(key)) {
						...
						if (matched === null || matched === void 0) continue;
						return dispatch(key, occupant, {
							...owner,
							matched
						}, opts);
					}
		...
		//#endregion
		//#region \0dsh-css:E:\dev\dsh-split-panes\src\client\PaneBody.module.css.mjs
		const css$1 = ".CGJjnW_body{flex-direction:column;flex:auto;min-height:0;display:flex}.CGJjnW_viewport{flex:auto;min-height:0;overflow-y:auto}";
		const tagId$1 = "@dsh-external/dsh-split-panes/PaneBody.module.css";
		...
		var PaneBody_module_css_default = {
			"body": "CGJjnW_body",
			"viewport": "CGJjnW_viewport"
		};
		//#endregion
		//#region src/client/PaneBody.tsx
		/**
		...
		*/
		...
		const PaneBody = (0, react.memo)(function PaneBody(props) {
			const { ctx, sessionId } = props;
			const kit = buildPaneKit(ctx, sessionId);
			const host = createPaneRenderHost(ctx.slots, kit, ctx.locale);
			const header = sessionId === void 0 ? null : host.renderSlot("conversation.session.header", {}, { fallback: null });
			const body = sessionId === void 0 ? null : host.renderSlot("conversation.session", {}, { fallback: null });
			const dockEntries = sessionId === void 0 ? null : host.renderSlot("conversation.input.dock", {}, { fallback: null });
			const composer = host.renderSlot("conversation.composer.bar", { variant: "composer" }, { fallback: null });
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: PaneBody_module_css_default.body,
				children: [
					header,
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: PaneBody_module_css_default.viewport,
						children: body
					}),
					dockEntries,
					composer
		...
		*/
		function apply(ctx) {
			...
			const usePaneStore = (selector) => bindSelector({
			...
			*/
			...
			const splitFocused = (direction) => {
				...
				if (paneId === void 0) return;
				const anchor = state.root.type === "leaf" ? ctx.sessions.list.getSnapshot().current ?? null : null;
				paneActions.splitPane(paneId, direction, anchor);
			};
			const closeFocused = () => {
				const state = paneTree();
			...
			const renderPane = (sessionId, key) => (0, react.createElement)(PaneBody, {
				key,
				ctx,
				sessionId
			});
