	...
	factory: (require) => {
		...
		*/
		function buildPaneKit(ctx, sessionId) {
			...
			if (ctx.workspaces?.list !== void 0) hooks["workspaces"] = bindSelector(ctx.workspaces.list);
			if (binding !== void 0) {
		...
		*/
		function resolveOccupant(raw) {
			...
			const render = (ctx, renderChild) => {
				...
				for (const [name, hook] of Object.entries(ctx.kit.hooks)) props[name.startsWith("use") ? name : (0, _deepseek_ai_dsh_client_ui_slots.standardHookPropName)(name)] = hook;
			...
			let renderChild = {
				renderSlot: (key, owner, opts) => {
					...
					if (opts?.entryKey !== void 0) {
						...
						return found === void 0 ? opts.fallback ?? null : found.render(mount, renderChild);
					}
					if (opts?.only !== void 0) {
					...
					const first = list[0];
					return first === void 0 ? opts?.fallback ?? null : first.render(mount, renderChild);
				},
				renderSlotChain: (key, owner, opts) => {
					for (const occupant of occupants(key)) {
						...
						return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PaneBoundary, {
							...
							children: occupant.render(mount, renderChild)
						});
					}
